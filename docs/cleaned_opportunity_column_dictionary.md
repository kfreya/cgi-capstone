...
`service_solution_estimated_revenue` remains separate and is not summed with
either opportunity-level revenue field. Kian still owns validation and
reliability review of the revenue semantics.
...
| `total_estimated_revenue` | revenue | Opportunity-level revenue field from the opps2/base side where available. | Primary input for `authoritative_revenue`. Kept as its own source column. |
| `opportunity_estimated_revenue_base_cad` | revenue | Opportunity-level CAD revenue field from the opps1 side where available. | Fallback input for `authoritative_revenue` when `total_estimated_revenue` is missing. Also merged onto matched opps1/opps2 rows when available. |
| `authoritative_revenue` | revenue | Non-additive opportunity-level revenue fallback field. | Uses `total_estimated_revenue` first, otherwise `opportunity_estimated_revenue_base_cad`. Does not use or sum `service_solution_estimated_revenue`. |
...
| `missing_revenue_flag` | data quality flag | True when both `total_estimated_revenue` and `opportunity_estimated_revenue_base_cad` are missing, where those fields exist. | Marks rows with no opportunity-level revenue candidate for `authoritative_revenue`. |

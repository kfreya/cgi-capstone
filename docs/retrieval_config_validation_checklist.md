...
| `python scripts/smoke_azure_chroma.py` | Pass | The smoke script now runs end to end and reports `stored_chunks: 3`, `query_result_count: 2`, and `first_result_mentions_cloud: True`. | No |
...
- The scripted smoke test now also passes, so the backend retrieval path is validated from config through Chroma smoke output.

...
-   Risk flags now use severity-appropriate Streamlit components:
    ...
    severity.
-   The upload widget now accepts TXT, DOCX, and text-based PDF files and
    loads extracted text into the RFP text area. Users can still paste or edit
    RFP text manually.
...
-   Displays: RFP summary, Estimated Effort (badge + duration + reason),
    Retrieved Supporting Examples (retrieved examples), Recommended Directors
...
-   Based on word count, service area count, complexity keyword hits,
    and best retrieval match score. Reason string names all inputs
    explicitly.
...
-   States the capacity label, top match score (to 2 d.p.), and
    either the service keyword overlap score or a note that director
    service-domain overlap is unavailable in the current capacity records.
-   **Assessment:** Grounded in observable signals. No overclaiming.

**LLM path** — generated by `chat_completion()`:

-   Requests a concise explanation for each candidate director.
    Injected into the recommendation via `_inject_llm_match_reasons()`.
-   Week 5 usability update: when real director records are used, the
    injected LLM match reason is wrapped with a prototype-fit caveat.
    This prevents LLM wording from implying that retrieved chunks prove a
    director's prior experience.
-   **Assessment:** More readable than the heuristic path, but still
    prototype guidance. It is explicitly not proof of prior director
    experience or a validated staffing decision.
...
-   States that experience fit is estimated from "retrieved
    historical/sample RFP chunks." When director service fields are
    available, it also shows simple service-keyword overlap. When those
    fields are missing, it states that overlap is unavailable.
-   **Assessment:** Honest. The word "sample" signals corpus type
    correctly, and unavailable service-domain metadata is no longer shown
    as a false `0.00` score.
...
Week 5 usability update: the app now displays a separate LLM enrichment
caption above the results. This makes it clear that retrieval fallback
and Azure OpenAI chat enrichment are separate states.
...
|-------------------------|-----------------|-------------------------------|
| `_match_reason` | "retrieved supporting examples" | Acceptable as semantic retrieval evidence; the UI now also warns that retrieved chunks do not prove director experience |
...
| `_notes` (LLM path) | "enriched with Azure OpenAI chat analysis. Retrieval remains local fallback." | Correct — explicitly separates LLM enrichment from retrieval mode |
| LLM match_reason | LLM-generated director rationale | Wrapped with a prototype-fit caveat so it does not imply proven prior director experience |

After the Week 5 wording pass, no unsupported claims were identified in
the current engine output. Capacity scores, match scores, and keyword
overlap are traceable to observable inputs. LLM-generated director text
is caveated as prototype fit and does not claim Azure-backed retrieval,
validated historical proposals, exact utilization, calendar availability,
or business-approved ranking weights.
...
-   [x] Effort estimate reason names its exact inputs (word count,
    ...
    path
-   [x] `match_reason` shows numeric signals (match score, keyword
...
Placeholder sections: Estimated Effort, Retrieved Supporting Examples,
...
-   File upload supports TXT, DOCX, and text-based PDF extraction. Scanned
    image PDFs may not produce readable text and should be pasted manually or
    OCR-processed outside the app.
...
-   The upload workflow is labelled with a caption explaining that users can
    upload a supported document or paste RFP text directly.
